...
def cal_loc(first_commit, second_commit, db_name, action):
    ...
    if sys.platform == "win32":
        command = f"powershell -c docker run --rm -v ${{PWD}}:/tmp aldanial/cloc --match-d='/src/sqlancer' --match-f='{db_name}.' --hide-rate --quiet --diff {first_commit} {second_commit}"
    else:
        command = f"docker run --rm -v ${{PWD}}:/tmp aldanial/cloc --match-d='/src/sqlancer' --match-f='{db_name}.' --hide-rate --quiet --diff {first_commit} {second_commit}"
...
if __name__ == '__main__':
    ...
    command = ""
    if sys.platform == "win32":
        command = f"""powershell -c docker run --rm -v ${{PWD}}:/tmp aldanial/cloc --match-d='/src/sqlancer' --not-match-f='(SQLite3^|MySQL^|Postgres).' --hide-rate --quiet --diff {first_commit} {second_commit}"""
    else:
